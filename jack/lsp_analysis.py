    ...
    def analyze(
        ...
        *,
        ...
        for path, source in project_sources.items():
            try:
                statements = parse(source, source_path=path)
            except ParseError:
                continue
            declaration = next(
        ...
        for entry in [focus, *(path for path in files if path != focus)]:
            ...
            graph_model = _GraphIndexBuilder(graph, overlays).build()
    ...
    def _statement(
        ...
        if isinstance(node, Assignment):
            if isinstance(node.name, str):
                ...
                self._expression(node.name, env)
            self._expression(node.expr, env)
            return
        ...
        if isinstance(node, If):
            for branch in node.branches:
                self._expression(branch.condition, env)
        ...
        if isinstance(node, While):
            self._expression(node.condition, env)
            child = self._child_scope(node.span, scope)
            child_env = dict(env)
